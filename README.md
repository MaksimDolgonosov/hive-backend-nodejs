...
> **Change Streams** (автоочистка ульев при TTL) требуют replica set. Локальный Docker MongoDB работает в standalone-режиме — сервер стартует нормально, но очистка ульев при TTL работает только на **MongoDB Atlas** (там replica set из коробки). При ручном удалении жала ульи обновляются сразу.
